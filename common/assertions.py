...
class Assertions:
    ...
    def contains_assert(self, value, response, status_code):
        ...
        for assert_key, assert_value in value.items():
            if assert_key == "status_code":
                ...
                if resp_list and isinstance(resp_list[0], str):
                    resp_list = ''.join(resp_list)
                if resp_list:
                    if assert_value is not None and isinstance(assert_value, str) and assert_value.upper() == 'NONE':
                        assert_value = None
    ...
    def equal_assert(self, expected_results, actual_results, statuc_code=None):
        ...
        if isinstance(actual_results, dict) and isinstance(expected_results, dict):
            ...
            if not common_keys:
                flag += 1
                logs.error(f"相等断言失败：预期结果的key {list(expected_results.keys())} 在实际响应中不存在")
                allure.attach(f"预期结果：{str(expected_results)}\n实际结果：{actual_results}", '相等断言结果：失败',
                              attachment_type=allure.attachment_type.TEXT)
                return flag
            # 根据共同的key构建实际结果子字典，与预期结果完整比较
            new_actual_results = {k: actual_results[k] for k in common_keys}
    ...
    def not_equal_assert(self, expected_results, actual_results, statuc_code=None):
        ...
        if isinstance(actual_results, dict) and isinstance(expected_results, dict):
            ...
            common_keys = list(expected_results.keys() & actual_results.keys())
            if not common_keys:
                # 没有共同key，说明确实不相等，断言成功
                logs.info(f"不相等断言成功：预期结果的key {list(expected_results.keys())} 在实际响应中不存在")
                return flag
            # 根据共同的key构建实际结果子字典，与预期结果完整比较
            new_actual_results = {k: actual_results[k] for k in common_keys}
            ...
            for yq in expected:
                for key, value in yq.items():
                    if key == "contains":
                        ...
                        # response_time 断言需要实际响应时间，从外部传入
                        # YAML中格式: response_time: 2 (单位：秒)
                        # 此处仅记录，实际比较在调用方完成
                        logs.info("响应时间断言阈值：%ss" % value)
                    else:
