    ...
    def specification_yaml(self, base_info, test_case):
        ...
        try:
            ...
            if base_info.get('cookies') is not None:
                cookie_str = self.replace_load(base_info['cookies'])
                try:
                    cookie = json.loads(cookie_str)
                except (JSONDecodeError, TypeError):
                    cookie = ast.literal_eval(cookie_str)
            ...
            try:
                ...
                validation = ast.literal_eval(test_case.pop('validation'))
            # 处理参数提取
